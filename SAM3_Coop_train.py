...
# import all utils
from utils import sam3_repo_dir, \
                    data_directory

sys.path.insert(0, os.path.join(sam3_repo_dir, "sam3"))
...
# torch.save(model.state_dict(), "weights/sam3.pth")
...
inputs = sam3_processor(images=image, text="ear", return_tensors="pt").to(device)
    ...
    while i < len(names):
        ...
        for name in batch_files:
            ...
            for file in name_files:
                ...
                combined_auth_mask.append(me["auth_mask"])
                
                combined_forged_mask.append(me["forged_mask"])
            
            # Combine masks by taking logical OR across entities
            combined_auth_mask = np.logical_or.reduce(combined_auth_mask)
            # combined_auth_mask = combined_auth_mask
...
for img_batch, auth_masks, forged_masks, _ in get_batches(batch_size=BATCH_SIZE):
    ...
    inputs = sam3_processor(images=img_batch, text=["copy and pasted image"]*len(img_batch), return_tensors="pt").to(device)
